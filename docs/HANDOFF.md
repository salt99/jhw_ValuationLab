...
**훅 발화는 아직 미확인이다.** 스크립트 자체는 파이프 테스트로 네 경로를 검증했고
(`.claude/hooks/*.sh`), 설정은 `jq -e` 스키마 검증을 통과했으며, 직전 세션에서 `/hooks` 를
한 번 열어 설정을 리로드했다. 하지만 `SessionStart` 는 세션이 시작될 때만 발화하므로
그 세션 안에서는 확인할 방법이 없었다.

→ **다음 세션이 확인할 것**: 이 문단이 별도 요청 없이 컨텍스트에 들어와 있으면
`SessionStart` 훅이 동작하는 것이다. 안 들어와 있으면 `/hooks` 에서 등록 여부를 보고,
그래도 없으면 `.claude/settings.json` 의 `${CLAUDE_PROJECT_DIR:-.}` 경로 해석을 의심할 것.
