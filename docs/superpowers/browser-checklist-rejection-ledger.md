...
125개 항목이 표로 출력된다. 데이터 무결성은 전부 여기서 확인된다:
...
- [ ] **시나리오 3행(목표가+확률)이 폰 폭에서 안 깨진다** — 가로 스크롤이 안 생긴다 (P3).
      `.prob-in{width:64px}`가 `.fld input{width:100%}`에 밀려(특정성 0,1,0 vs 0,1,1) 실제로는
      목표가·확률이 나란히가 아니라 **위아래로 쌓인 동일 너비 입력 두 칸**으로 렌더된다
      (측정: 추가 폼 134/134px, 수정 모달 95/95px, 재평가 모달 402/402px). CSS를 고치라는
      게 아니라 — 이 모양 그대로 폰에서 **어느 칸이 목표가고 어느 칸이 확률인지 라벨만
      보고 헷갈리지 않는지**를 확인한다
